...
static2 int block2buff(IDESC*, int, IDESC*, int, double*, MDESC*, double*);
static2 void buff2block(IDESC*, int, IDESC*, int, double*, double*, MDESC*);
...
//WCC  double *ptrmyblock, *ptrmynewblock;
...
after_comm:
  ...
  if (myprow1 >= 0 && mypcol1 >= 0) {
    freememory((double *) ptrrecvbuff);
  };
  if (myprow0 >= 0 && mypcol0 >= 0) {
    freememory((double *) ptrsendbuff);
  };
  if (nprow != 1)
    Cblacs_gridexit(gcontext);
  free(v_inter);
