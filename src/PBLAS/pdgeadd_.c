...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 801 + CTXT_ ) : 0 ) ) )
   {
      ...
      {
         PB_Cwarn( ctxt, __LINE__, "PDGEADD", "Illegal TRANS = %c\n", TrA );
         info = -1;
      }
      if( notran )
         PB_Cchkmat( ctxt, "PDGEADD", "A", *M, 2, *N, 3, Ai, Aj, Ad,  8,
                     &info );
      else
         PB_Cchkmat( ctxt, "PDGEADD", "A", *N, 3, *M, 2, Ai, Aj, Ad,  8,
                     &info );
      PB_Cchkmat( ctxt, "PDGEADD", "C", *M, 2, *N, 3, Ci, Cj, Cd, 13, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PDGEADD", info ); return; }
#endif
/*
...
*/
   if( ALPHA[REAL_PART] == ZERO )
   {
      ...
      {
         PB_Cplapad( PB_Cdtypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                     ((char *)BETA), ((char *) C), Ci, Cj, Cd );
      }
      else
      {
         PB_Cplascal( PB_Cdtypeset(), ALL, NOCONJG, *M, *N, ((char *)BETA),
                      ((char * )C), Ci, Cj, Cd );
      }
      return;
   }
...
*/
   if( notran )
   {
      ...
      PB_Cpgeadd( PB_Cdtypeset(), &DirA, &DirC, NOCONJG, *M, *N,
                  ((char *) ALPHA), ((char *) A), Ai, Aj, Ad,
                  ((char *)  BETA), ((char *) C), Ci, Cj, Cd );
   }
   else
   {
      PB_Cptran( PB_Cdtypeset(), NOCONJG, *M, *N, ((char *) ALPHA),
