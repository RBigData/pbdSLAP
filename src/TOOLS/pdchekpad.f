      SUBROUTINE PDCHEKPAD( ICTXT, MESS, M, N, A, LDA, IPRE, IPOST,
     $                     CHKVAL )
*
...
*  =======
*
*  PDCHEKPAD checks that the padding around a local array has not
*  been overwritten since the call to PDFILLPAD.  3 types of errors
*  are reported:
*
...
*
*  A       (local input) DOUBLE PRECISION array of dimension (LDA,N).
...
*WCC           WRITE( *, FMT = 9998 ) MYROW, MYCOL, MESS, ' pre', I,
*WCC $                                A( I )
               INFO = IAM
            END IF
   10    CONTINUE
      ELSE
*WCC     WRITE( *, FMT = * ) 'WARNING no pre-guardzone in PDCHEKPAD'
      END IF
*
...
*
      IF( IPOST.GT.0 ) THEN
         ...
         DO 20 I = J, J+IPOST-1
            IF( A( I ).NE.CHKVAL ) THEN
*WCC           WRITE( *, FMT = 9998 ) MYROW, MYCOL, MESS, 'post',
*WCC $                                I-J+1, A( I )
               INFO = IAM
            END IF
   20    CONTINUE
      ELSE
*WCC     WRITE( *, FMT = * )
*WCC $          'WARNING no post-guardzone buffer in PDCHEKPAD'
      END IF
*
...
*
      IF( LDA.GT.M ) THEN
         ...
         DO 40 J = 1, N
            DO 30 I = K, K + (LDA-M) - 1
               IF( A( I ).NE.CHKVAL ) THEN
*WCC              WRITE( *, FMT = 9997 ) MYROW, MYCOL, MESS,
*WCC $               I-IPRE-LDA*(J-1), J, A( I )
                  INFO = IAM
               END IF
   30       CONTINUE
            K = K + LDA
...
*WCC     WRITE( *, FMT = 9999 ) INFO / NPCOL, MOD( INFO, NPCOL ), MESS
      END IF
*
