 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         //WCC ierr=MPI_Op_create(BI_dMPI_amn2, i, &BlacComb);
         //WCC:add
         ierr=MPI_Op_create((MPI_User_function *) BI_dMPI_amn2, i, &BlacComb);
      }
      else
      {
         //WCC ierr=MPI_Op_create(BI_dMPI_amn, i, &BlacComb);
         //WCC:add
         ierr=MPI_Op_create((MPI_User_function *) BI_dMPI_amn, i, &BlacComb);
